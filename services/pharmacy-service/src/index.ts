...
import { auth as SharedAuth } from "@clinic/shared";

const app = express();
app.use(express.json());

const PORT = Number(process.env.PHARMACY_SERVICE_PORT || 4100);
const IDENTITY_BASE = process.env.IDENTITY_BASE_URL || "http://identity-service:4000";
...
app.post("/pharmacy/verify-staff", async (req: Request, res: Response) => {
  ...
  const claims = SharedAuth.decodeToken(token);
